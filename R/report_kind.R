...
report_kind <- function(paper,
  ...
  # set up report ----
  report_text <- report_qmd_kind(module_output, paper)
...
#' @returns report text
#' @export
report_qmd_kind <- function(module_output, paper = list()) {
  ## read in report template ----
  report_template <- system.file("templates/_report_kind.qmd",
    package = "metacheck"
  )
