...
report_harsh <- function(paper,
  ...
  # set up report ----
  report_text <- report_qmd_harsh(module_output, paper)
...
#' @returns report text
#' @export
report_qmd_harsh <- function(module_output, paper = list()) {
  ## read in report template ----
  report_template <- system.file("templates/_report_harsh.qmd",
    package = "metacheck"
  )
