...
zenodo_links <- function(paper) {
  ...
  links$zenodo_id <- zenodo_extract_id(links$zenodo_url)
...
zenodo_extract_id <- function(zenodo_url) {
  if (length(zenodo_url) == 0) {
    return(character(0))
  }

  if (length(zenodo_url) > 1) {
    return(vapply(zenodo_url, zenodo_extract_id, character(1)))
  }
...
#'   zenodo_retrieve("https://doi.org/10.5281/zenodo.18648142")
#' }
zenodo_retrieve <- function(zenodo_url, id_col = 1, pb = NULL) {
  ...
  ids <- data.frame(
    ...
    zenodo_id = zenodo_extract_id(table$zenodo_url),
...
zenodo_info <- function(zenodo_id, pb = NULL) {
  ...
  zenodo_id <- zenodo_extract_id(zenodo_id)
  ...
  # Basic metadata
  obj$title <- if (is.null(metadata[["title"]])) NA_character_ else metadata[["title"]]
  obj$doi <- if (is.null(rec[["doi"]])) NA_character_ else rec[["doi"]]
  obj$description <- if (is.null(metadata[["description"]])) NA_character_ else metadata[["description"]]
  obj$publication_date <- if (is.null(metadata[["publication_date"]])) NA_character_ else metadata[["publication_date"]]
  obj$updated_date <- if (is.null(rec[["updated"]])) NA_character_ else rec[["updated"]]
  ...
  obj$license <- license
  obj$downloads <- if (is.null(stats[["downloads"]])) NA_real_ else stats[["downloads"]]
  obj$unique_downloads <- if (is.null(stats[["unique_downloads"]])) NA_real_ else stats[["unique_downloads"]]
  obj$views <- if (is.null(stats[["views"]])) NA_real_ else stats[["views"]]
...
zenodo_file_download <- function(zenodo_id,
  ...
  paste0("Starting retrieval for ", zenodo_id) |>
    list(what = _) |>
    pb$tick(0, tokens = _)
  contents <- suppressMessages(zenodo_retrieve(zenodo_id, pb = pb))
  ...
  rows <- lapply(
    ...
    function(x) {
      tibble::tibble(
        id       = if (is.null(x$id)) NA_character_ else x$id,
        key      = if (is.null(x$key)) NA_character_ else x$key,
        size     = if (is.null(x$size)) NA_real_ else as.numeric(x$size),
        checksum = if (is.null(x$checksum)) NA_character_ else x$checksum,
        self     = if (is.null(x$links) || is.null(x$links$self)) NA_character_ else x$links$self
      )
    }
  )
