...
# the scienceverse DB: one row per extracted statistic, carrying its STATO type.
# The document is metacheck's native schema (R/stat-output.R's
...
.sv_statistical_results <- function(doi, stat_out) {
  ...
  # Normalise to a list of documents: a single document is recognised by its
  # own self-identifying `schema` field (or by carrying `analyses` directly).
  ...
  for (doc in docs) {
    ...
    for (an in doc[["analyses"]]) {
      ...
      for (res in an[["results"]] %||% list()) {
        ...
        result <- if (nzchar(rid)) rid else rlab
