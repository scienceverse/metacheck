...
ES_experiments <- 
  ...
  mutate(
    ...
    blind_lab = factor(blind, levels = 0:1, labels = c("Unblind","Blind")),
...
```
```{r}
ggsave("figures/Figure-1.eps", width = 180, height = 140, units = "mm", scale = 1.3)
ggsave("figures/Figure-1.pdf", width = 180, height = 140, units = "mm", scale = 1.3)
ggsave("figures/Figure-1.svg", width = 180, height = 140, units = "mm", scale = 1.3)
```
...
RMA_study_only <- rma.mv(d ~ 0 + con_lab, V = v,
                        random = ~ 1 | study, method = "REML",
                        data = ES_experiments)
vc_within_p <- format.pval(anova(RMA_study_het, RMA_study_only)$pval, eps = 1e-4)

RMA_es_only <- rma.mv(d ~ 0 + con_lab, V = v,
                        random = ~ 1 | esID, method = "REML",
                        data = ES_experiments)
vc_between_p <- format.pval(anova(RMA_study_het, RMA_es_only)$pval, eps = 1e-4)
...
One way to assess replicability is to examine the consistency of effect size estimates generated by the initial confirmatory test and the subsequent replications of the same phenomenon (effect/study) Based on a multi-level meta-analysis, we found a small level of variation in effect sizes ($\hat\tau_{within}$ = `r round(vc_within_CI[["estimate"]], 3)`, 95% CI = `r round(vc_within_CI[["ci.lb"]], 3)` to `r round(vc_within_CI[["ci.ub"]], 3)`, $p `r vc_within_p`$) beyond what would be expected by sampling variation alone. This indicates that the self- and independent replications would not have perfectly replicated the effect size magnitude of the confirmatory tests, even if all studies were so large that sampling error was negligible. The degree of variation was, however, smaller than the variation in effect sizes across the sixteen conducted studies ($\hat\tau_{between}$ = `r round(vc_between_CI[["estimate"]], 3)`, 95% CI = `r round(vc_between_CI[["ci.lb"]], 3)` to `r round(vc_between_CI[["ci.ub"]], 3)`, $p `r vc_between_p`$).

## Alternative specification 

One alternative specification for the above model is to allow the intercepts and the between-study and within-study variance components to differ depending on whether the effect size was generated by the original lab or by an independent lab. At the between-study level, we allow the effect studied in the original lab to co-vary with the _average_ effect of the replicating labs. At the within-study level, we allow for random variation in the effect sizes of the confirmatory test and the self-replication, and then a separate degree of random variation in the effects sizes across the independent replications. The results of this model are given below.

```{r}
RMA_study_het_het <- 
  rma.mv(d ~ 0 + lab_match, V = v,
  random = list(~ other_rep_lab | study, ~ other_rep_lab | esID), 
  struct = c("CS","DIAG"), 
  method = "REML",
  data = ES_experiments)

RMA_study_het_null_rho <- update(RMA_study_het_het, struct = c("ID","DIAG"))
RMA_study_het_null_gamma2 <- update(RMA_study_het_het, gamma2 = c(NA, 0))

rho <- confint(RMA_study_het_het, rho = 1)$random["rho",]
rho_p <- format.pval(anova(RMA_study_het_het, RMA_study_het_null_rho)$pval, eps = 1e-4)
gamma1 <- confint(RMA_study_het_het, gamma2 = 1)$random["gamma.1",]
gamma2  <- confint(RMA_study_het_het, gamma2 = 2)$random["gamma.2",]
gamma2_p <- format.pval(anova(RMA_study_het_het, RMA_study_het_null_gamma2)$pval, eps = 1e-4)


robustify(RMA_study_het_het)
```

In a further, exploratory model, we found that differences between confirmatory test and self-replication effect sizes was fully attributable to sampling error; that average effects in independent replications were `r round(100 * RMA_study_het_het$beta[3] / RMA_study_het_het$beta[1])`% of the size of average effects in confirmatory tests; that average effects in independent replications correlated at r = `r round(rho[["estimate"]], 3)` (95% CI = `r round(rho[["ci.lb"]], 3)` to `r round(rho[["ci.ub"]], 3)`) with the effect sizes from confirmatory and self-replication studies; and that there was a small amount of heterogeneity in effect size estimates across the independent replications ($\hat\tau_{within}$ = `r round(gamma2[["estimate"]], 3)`, 95% CI = `r round(gamma2[["ci.lb"]], 3)` to `r round(gamma2[["ci.ub"]], 3)`). These results indicate that heterogeneity of effects may be attributed partly to the challenge of communicating study methods to the independent replicating labs and partly to varying interpretation or implementation of the study methods by replicating labs.
...
Based on the 13 confirmatory tests with statistically-significant results, average power was `r round(power_dist$mean[2], 3)` with a median of `r round(power_dist$median[2], 3)` and a range of `r round(power_dist$min[2], 3)` to `r round(power_dist$max[2], 3)`. The observed positive result rate of `r round(rep_rate_sig[2] * 100)`% is somewhat smaller than expected based on these power estimates. For all confirmatory studies including statistically non-significant ones, the average power was `r round(power_dist$mean[1], 3)` with a median of `r round(power_dist$median[1], 3)` and a range of `r round(power_dist$min[1], 3)` to `r round(power_dist$max[1], 3)`. The observed replication rate of `r sig_rep_rate`% in the replication studies for all 16 original hypotheses was somewhat larger than expected based on power estimates. Overall, replication rates were consistent with power estimates calculated using the ESs observed in confirmatory tests.
...
aggregate_replications <- 
  ES_experiments %>%
  group_by(lab, study, lab_study, lab_match) %>%
  summarise(
    blind = mean(blind),
    d = weighted.mean(d, w = 1 / v),
    v = 1 / sum(1 / v),
    n = sum(n)
  ) %>%
  mutate(
    pval = 2 * pnorm(abs(d / sqrt(v)), lower.tail = FALSE),
    lab_match = fct_recode(lab_match, conf = "Confirmatory test", self = "Self-replication", indep = "Independent replication")
  ) %>%
  pivot_wider(names_from = lab_match, values_from = c(d, v, n, pval)) %>%
  mutate(
    d_allrep = (d_self / v_self + d_indep / v_indep) / (1 / v_self + 1 / v_indep),
    v_allrep = 1 / (1 / v_self + 1 / v_indep),
    n_allrep = n_self + n_indep,
    pval_allrep = 2 * pnorm(abs(d_allrep / sqrt(v_allrep)), lower.tail = FALSE) 
  ) %>%
  ungroup() %>%
  arrange(desc(lab), study) %>%
  select(lab, study, ends_with("conf"), ends_with("allrep"), ends_with("self"), ends_with("indep"))

write_csv(aggregate_replications, "Confirmation and replication studies - aggregated effect sizes.csv")
...
rep_diffs_long_plus <- 
  ...
  bind_rows(meta_res) %>% 
  mutate(
    type = factor(type, levels = c("indep","self"), labels = c("Independent replication","Self-replication")),
...
```{r}
ggsave("figures/Figure-2.eps", width = 80, height = 144, units = "mm", scale = 1.8)
ggsave("figures/Figure-2.pdf", width = 80, height = 144, units = "mm", scale = 1.8)
ggsave("figures/Figure-2.svg", width = 80, height = 144, units = "mm", scale = 1.8)
```

### Self-replications meta-analysis

```{r}
self_diff_tau_CI <- confint(self_diff_meta)$random["tau",-1]
self_diff_tau_p <- format.pval(anova(self_diff_meta, update(self_diff_meta, method = "FE"))$pval, eps = 1e-4)
summary(self_diff_meta)
```

On average, the self-replications were the same size as the confirmatory tests ($d_{difference}$ = `r round(as.numeric(self_diff_meta$beta), 3)`, p = `r round(self_diff_meta$pval,3)`, 95% CI =  `r round(self_diff_meta$ci.lb,3)` to `r round(self_diff_meta$ci.ub,3)`, between study heterogeneity $\hat\tau$ = `r round(sqrt(self_diff_meta$tau2),3)`, 95% CI = `r round(self_diff_tau_CI["ci.lb"], 3)` to `r round(self_diff_tau_CI["ci.ub"],3)`).
...
indep_diff_tau_p <- format.pval(anova(indep_diff_meta, update(indep_diff_meta, method = "FE"))$pval, eps = 1e-4)
...
```
```{r}
ggsave("figures/Figure-3.eps", width = 180, height = 150, units = "mm", scale = 1.2)
ggsave("figures/Figure-3.pdf", width = 180, height = 150, units = "mm", scale = 1.2)
ggsave("figures/Figure-3.svg", width = 180, height = 150, units = "mm", scale = 1.2)
```
