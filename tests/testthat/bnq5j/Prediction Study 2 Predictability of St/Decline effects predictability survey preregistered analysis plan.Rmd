...
subtitle: "Pre-registered analysis plan, Prolific pilot data"
...
predict_survey <- 
  read_survey("Raw data/Decline Effect Generalizability - pilot data.csv") %>%
  select(
    StartDate, EndDate,  
    Finished, serious,
  ...
  # Limit to Prolific respondents
  filter(StartDate >= ymd("2022-10-17"))

```

The remainder of this pre-registered analysis plan is compiled using data from a pilot survey conducted on October 17, 2022. 
The pilot data is excluded from final analysis.
...
Based on our pre-specified analysis plan, we estimate that the average predictability of Decline Effect prompts differed from that of the Comparison Effect prompts by `r pct_correct %>% filter(effect == "Decline - Comparison") %>% pull(est_CI)`. Under the null of $\delta_0 = 5$, this result corresponds to $p `r pct_correct_p`$.
