test_that("text_search", {
  expect_true(is.function(metacheck::text_search))
  expect_no_error(helplist <- help(text_search, metacheck))

  expect_error(text_search(1))
...
test_that("search_text alias", {
  ...
  paper <- demopaper()
  st <- search_text(paper, "significant")
  ts <- text_search(paper, "significant")
...
test_that("table as first argument", {
  ...
  sig <- text_search(paper, "significant")
  sig2 <- text_search(sig, "significant")
  expect_equal(sig, sig2)

  s3 <- text_search(sig, "significant", return = "paper_id")
...
test_that("return", {
  ...
  res_s1 <- text_search(s)
  res_s2 <- text_search(s, return = "sentence")
  res_p <- text_search(s, return = "paragraph")
  res_div <- text_search(s, return = "header")
  res_m <- text_search(s, "Part [0-9]", return = "match")
  res_id <- text_search(s, return = "paper_id")
...
test_that("odd errors", {
  ...
  pattern <- "this"
  x <- text_search(paper, pattern, return = "match")
  ...
  pattern <- "significant"
  x <- text_search(paper, pattern, perl = TRUE, return = "match")
...
test_that("exclude", {
  ...
  pattern <- c("mango")
  x <- text_search(paper, pattern, exclude = TRUE)
...
#
#   pattern <- c("apple")
#   x <- text_search(paper, pattern, search_header = TRUE)
...
#
#   pattern <- c("No")
#   x <- text_search(paper, pattern, exclude = TRUE, search_header = TRUE)
#   expect_equal(x$text, text[1:3])
# })
