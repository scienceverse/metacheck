test_that("exists", {
  expect_true(is.function(zenodo_links))

  expect_true(is.function(zenodo_retrieve))

  expect_true(is.function(zenodo_info))

  expect_true(is.function(zenodo_file_download))
})


test_that("errors", {
  expect_error(zenodo_links(bad_arg))
  expect_error(zenodo_retrieve(bad_arg))
...
test_that("zenodo_extract_id", {
  ...
  ids <- zenodo_extract_id(inputs)
  expect_equal(unname(ids), c("12345", "12345", "12345", "12345", "12345", NA))
})
...
test_that("zenodo_retrieve", {
  ...
  info2 <- zenodo_retrieve(tbl, "href")
...
test_that("zenodo_file_download", {
  testthat::local_mocked_bindings(
    zenodo_retrieve = function(zenodo_url, id_col = 1, pb = NULL) {
      zid <- as.character(zenodo_extract_id(zenodo_url))
      data.frame(
