...
Create a data frame with info about each paper. One column must be called "xml" and contain paths to the xml files (relative to the validation file location). Other possible columns are "table", "report", and "traffic_light", which should contain the expects values of those items from the module you're testing. If you want to check more than just the first row of the text column from the return table, omit the table column and use the method in the next section.
...
```{r}
v$results_table |>
  dplyr::filter(xml == "xml/prereg.xml")
```
