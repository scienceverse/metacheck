...
The `table` is usually a detailed table in the format returned from `search_text()` or `expand_text()`, containing either text relevant to the module, or a classification of the text. This table can be of use to further modules in a chain, or to metascientific users. 
...
Flag any cited papers in the RetractionWatch database. It takes a substantial time to make calls to crossref for each reference, so we provide a summary of the database in metacheck, which is updated regularly (last 2025-02-28). 
