...
There are some built-in functions in papercheck for exploring OSF repositories. You can use these in custom modules.
...
We start by searching for all URLs by using "all_urls" module, then narrowing that down to just the probable links to the Open Science Framework (OSF).

```{r search_osf}
mod <- module_run(paper, "all_urls")
osf_links <- mod$table |>
  filter(grepl("osf\\s*\\.\\s*io", text, ignore.case = TRUE))
```
...
If found, the link is processed, and the OSF Application Programming Interface (API) is used to retrieve whether the link points to a file, project, or registration, and what the overarching project is, in case this is not the folder linked to. This is achieved through the `osf_retrieve()` function.

This function can take a vector of OSF IDs or URLs, or a table that contains them. If the IDs aren't in the first column, you will need to specify the name of the column. The function will return your table with added information.

```{r enrich_links}
link_info <- osf_retrieve(osf_links)
```
...
There are multiple OSF links in this paper, but they are all part of the same overarching OSF project, with the project ID *`r link_info$project[[1]]`*. 

### Get all Project Contents

The function `osf_retrieve()` will also retrieve all child components, files and folders if you set the argument `recursive = TRUE`. We can retrieve all of the contents under the parent project(s) with the following code.

If you give this function a list with duplicate projects or missing values, it will only get the contents for each project once. 

```{r get_osf_files}
osf_files <- osf_retrieve(link_info$project, recursive = TRUE)
```

We can take a look at these results. The OSF allows you to categorize components by category, and we can also determine file types using extensions. It looks like these authors did not use any categories beyond the default project one. 

```{r}
#| echo: false
osf_files |>
  select(osf_id, name, osf_type, kind, filetype) |>
  mytable()
```
...
osf_files_summary <- summarize_contents(osf_files)
```
...
osf_report <- function(summary) {
  ...
  readme_report <- dplyr::case_when(
    readme == 0 ~ "\u26A0\uFE0F No README files were identified. A read me is best practice to facilitate re-use. If you have a read me, please name it explicitly (e.g., read_me.txt).",
    readme > 0 ~ "\u2705 Read Me detected. Great job making it easier to understand how to re-use files in your repository!"
  )
...
```{r}
#| results: 'asis'
osf_report(osf_files_summary) |>cat()
```
...
osf_file_check <- function(paper) {
  ...
  osf_files <- osf_retrieve(link_info$project, recursive = TRUE)
  osf_files_summary <- summarize_contents(osf_files)
  report_message <- osf_report(osf_files_summary)
  r_file_results <- check_r_files(osf_files_summary)  
