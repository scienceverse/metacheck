...
stat_p_exact <- function(paper) {
  ...
  if (tl == "na") {
    ...
    report <- sprintf(
      "We found no imprecise *p* values or *p*-values of exactly zero out of %d detected. Well done, you followed reporting practices perfectly.",
      nrow(p)
    )
    summary_text <- report
  } else {
    ...
    summary_text <- sprintf(
      "We found %s out of %d detected *p* value%s. You might want to review these to see if improvements can be made.",
