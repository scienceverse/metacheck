...
stat_p_exact <- function(paper) {
  ...
  if (tl == "na") {
    ...
    report <- sprintf(
      "We found no imprecise *p* values or *p*-values of exactly zero out of %d detected. No errors could be detected.",
      nrow(p)
    )
    summary_text <- report
  } else {
    ...
    summary_text <- sprintf(
      "We found %s out of %d detected *p* value%s. This is not good practice and should be corrected.",
