...
#'
#' <validation>In a sample of 250 papers with 309 instances of power statements, this module correctly indicated at least one aspect of the power analysis was missing in 91% of the power analyses. The module did not detect 7% of the power analysis statements in the papers.</validation>
#'
#' @keywords method
#'
...
power <- function(paper, seed = 8675309, think = NULL) {
  ...
  if (nrow(potential_power) > 0 && llm_use()) {
    ...
    if (nrow(table) == 0) {
      ...
      tl <- "green"
      report_text <- "No errors could be detected."
    } else {
      ...
      report_text <- sprintf(
        "Some essential information could not be detected, which is not best practice, and should be corrected: %s",
  ...
  if (nrow(table) == 0) {
    ...
    observed_power_text <- ifelse(
      ...
      observed_power_text <- "You reported a power analysis that has been classified as 'post-hoc'. This is not best practice. Calculating observed power is [almost never useful](https://lakens.github.io/statistical_inferences/08-samplesizejustification.html#sec-posthocpower). If you actually performed a sensitivity power analysis, label it as such explicitly.",
      ""
    )
