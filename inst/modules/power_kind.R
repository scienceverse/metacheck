...
power <- function(paper, seed = 8675309, think = NULL) {
  ...
  if (nrow(potential_power) > 0 && llm_use()) {
    ...
    if (nrow(table) == 0) {
      ...
      tl <- "green"
      report_text <- "All essential information could be detected.  Well done, you followed reporting practices perfectly."
    } else {
      ...
      report_text <- sprintf(
        "Some essential information could not be detected, and you might want to review the following information to see if improvements can be made: %s",
