...
#'
#' DOI, journal and year are reliable signals and flag a reference on their own. Title and author mismatches are more often caused by imperfect PDF parsing, so by default a reference is only flagged when at least `min_mismatches` (default 2) of these two disagree. Set `min_mismatches = 1` to also flag a lone title or author mismatch (higher recall, more false positives).
#'
