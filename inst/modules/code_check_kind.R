...
code_check <- function(paper, file_limit = 20) {
  ...
  if (length(library_issue) == 0) {
    report_library <- "Best programming practice is to load all required libraries/imports in one block near the top of the code. In all code files, libraries/imports were loaded in one block. Well done, you followed best practices perfectly."
    summary_library <- "All libraries/imports were loaded in one block. Well done, you followed best practices perfectly."
    report_table_library <- NULL
  } else {
    report_library <- sprintf(
      "Best programming practice is to load all required libraries/imports in one block near the top of the code. In %d code files, libraries/imports were at multiple places (i.e., with more than 3 non-comment lines in between). You might want to review the code to see if improvements can be made.",
      length(library_issue)
    )
    summary_library <- "Libraries/imports were loaded in multiple places. You might want to review the code to see if improvements can be made. "
  }

  ## absolute paths ----
  absolute_issues <- code_files$file_name[code_files$code_abs_path > 0]
  if (length(absolute_issues) == 0) {
    report_absolute <- "Best programming practice is to use relative file paths (e.g., './files') instead of absolute file paths (e.g., 'C://Lakens/project_dir/files') as these folder names do not exist on other computers. No absolute file paths were found in any of the code files.Well done, you followed best practices perfectly."
    summary_absolute <- "No absolute file paths were found. Well done, you followed best practices perfectly."
    report_table_absolute <- NULL
  } else {
    report_absolute <- sprintf(
      "Best programming practice is to use relative file paths instead of absolute file paths (e.g., C://Lakens/files) as these folder names do not exist on other computers. You might want to review the file paths in your code to see if improvements can be made. The following absolute file paths were found in %d code file%s:",
      length(absolute_issues),
      plural(length(absolute_issues))
    )
    summary_absolute <- "Absolute file paths were found. You might want to review the file paths in your code to see if improvements can be made."
  ...
  if (length(comment_issue) == 0) {
    report_comments <- "Best programming practice is to add comments to code, to explain what the code does (to yourself in the future, or peers who want to re-use your code). All your code files had comments. Well done, you followed reporting practices perfectly."
    summary_comments <- "All your code files had comments. Well done, you followed reporting practices perfectly."
  } else {
    report_comments <- "Best programming practice is to add comments to code, to explain what the code does (to yourself in the future, or peers who want to re-use your code). You might want to review your code files to see if improvements can be made. "
    summary_comments <- sprintf(
      "%d code file%s had no comments. You might want to review these to see if improvements can be made.",
  ...
  if (length(missingfiles_issue) == 0) {
    summary_missingfiles <- "All files loaded in the code were present in the repository. Well done, you followed best practices perfectly."
    ...
    summary_missingfiles <- sprintf(
      "%d file%s loaded in the code %s missing in the repository. You might want to review these files to see if improvements can be made. ",
      n_missing, plural(n_missing), plural(n_missing, "was", "were")
    )

    report_missingfiles <- sprintf(
      "The scripts load files, but %d script%s loaded %d file%s that could not be automatically identified in the repository. You might want to review these to see if improvements can be made. Check if the following files are made available, so that others can reproduce your code, or that the files are missing:",
