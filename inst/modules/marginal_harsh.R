...
marginal <- function(paper) {
  ...
  # summary_text ----
  summary_text <- sprintf("You described %d effect%s with terms related to 'marginally significant'. Any occurrances of terms related to 'marginal significance' should be corrected.", nrow(table), ifelse(nrow(table) == 1, "", "s"))


  # report ----
  if (tl == "green") {
    report <- "No effects were described with terms related to 'marginally significant'. No errors could be detected."
  } else if (tl == "red") {
    report_text <- "You described effects with terms related to 'marginally significant'. This is not good practice and should be corrected. If *p* values above 0.05 are interpreted as an effect, you inflate the alpha level, and increase the Type 1 error rate. If a *p* value is higher than the prespecified alpha level, it should be interpreted as a non-significant result."
