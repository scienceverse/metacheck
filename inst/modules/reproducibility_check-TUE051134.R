...
reproducibility_check <- function(paper, local_path = NULL, local_only = FALSE,
  ...
  # the scienceverse DB) and a full structured document (stat_output_json, for
  # the logs). This runs regardless of whether the paper also has R code.
  ...
  if (isTRUE(execute)) {
    ...
    # STATO typing + structured-document emitter. script_lines (the executed
    # script's own text, from repro_run_scripts()'s echo = TRUE run) lets
    # read_r_output() attach the source LINE each result came from. Appended to
    # stat_output alongside any JASP/jamovi results.
