...
stat_effect_size <- function(paper) {
  ...
  if (tl == "na") {
    ...
    report <- "All detected t-tests and F-tests had an effect size reported in the same sentence. No errors could be detected."
    summary_text <- report
  } else {
    summary_text <- sprintf(
      "We found %1$d t-test%2$s and/or F-test%2$s where effect sizes are not reported. This is not good practice and should be corrected.",
      nrow(table_missing),
