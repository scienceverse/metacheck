...
stat_effect_size <- function(paper) {
  ...
  if (tl == "na") {
    ...
    report <- "All detected t-tests and F-tests had an effect size reported in the same sentence.  Well done, you followed reporting practices perfectly."
    summary_text <- report
  } else {
    summary_text <- sprintf(
      "We found %1$d t-test%2$s and/or F-test%2$s where effect sizes are not reported. You might want to review these to see if improvements can be made.",
      nrow(table_missing),
