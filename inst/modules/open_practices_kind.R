...
open_practices <- function(paper) {
  ...
  if (nrow(summary_table) > 1) {
    ...
    summary_text <- sprintf(
      ...
      summary_text <- "Neither shared data nor code detected. You might want to review if any data or code can be made available."
      tl <- "red"
  } else {
    # summary for a single paper
    if (summary_table$data_open == TRUE &
        summary_table$code_open == TRUE) {
      summary_text <- "Shared data and code detected.  Well done, you followed best practices perfectly."
      tl <- "green"
  ...
  if (nrow(summary_table) == 1) {
    ...
    if (!summary_table$data_open) {
      data_report <- "We did not detect open sharing of data, which could be because there is no data related to this article, or the repository is not recognized by our code. You might want to review if any data or code can be made available."
    } else {
      data_report <- sprintf(
    ...
    if (!summary_table$code_open) {
      code_report <- "We did not detect open sharing of code, which could be because there is no code related to this article, or the repository is not reconized by our code. You might want to review if any data or code can be made available."
    } else {
      code_report <- sprintf(
    ...
    if (summary_table$on_request) {
      on_request_report <- sprintf(
        "Some materials are shared on request, but sharing only upon request can make it difficult - especially further in the future - for others to receive the data. Maybe you can consider making materials publicly available? This is based on the following text:\n\n> %s",
